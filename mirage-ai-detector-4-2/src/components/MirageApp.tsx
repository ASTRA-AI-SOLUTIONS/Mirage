...
export const MirageApp = () => {
  const { isBubbleEnabled, setIsBubbleEnabled, isWidgetEnabled, setIsWidgetEnabled, analyzeMediaContent, analyzingState, history, selectedModel, setSelectedModel } = usePhone();
          ...
          {activeTab === 'scanner' && (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/5 rounded-3xl p-5 flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-medium text-white/90">Scroll Down Widget</div>
                      <div className="text-xs text-white/50 font-light">Add to notification quick settings</div>
                    </div>
                    <button 
                      className={`w-12 h-6 rounded-full transition-colors relative ${isWidgetEnabled ? 'bg-white/20' : 'bg-white/5'}`}
                      onClick={() => setIsWidgetEnabled(!isWidgetEnabled)}
                    >
                      <motion.div 
                        className="w-5 h-5 bg-white rounded-full absolute top-0.5 shadow-sm"
                        animate={{ left: isWidgetEnabled ? '26px' : '2px' }}
                        transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                      />
                    </button>
                  </div>
