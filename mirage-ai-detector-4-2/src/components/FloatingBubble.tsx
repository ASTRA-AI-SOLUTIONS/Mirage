...
export const FloatingBubble = () => {
  ...
  // Hide the web bubble on native Android to prevent duplicate bubbles
  if (!isBubbleEnabled || Capacitor.isNativePlatform()) return null;
  ...
  const handleAnalyze = async () => {
    ...
    if (!visiblePost) {
      alert("No visible post found to analyze. Please open the Social app and scroll to a post.");
      setIsOpen(false);
      return;
    }
    ...
    try {
      if (video) {
        setAnalyzingState({ isAnalyzing: true, result: null });
        const frames = await captureLiveVideoFrames(video, 5, 200); // Match native 5 frames / 200ms
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-40" ref={constraintsRef}>
      ...
      >
        <motion.div 
          className={`w-12 h-12 backdrop-blur-xl rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.2)] flex items-center justify-center cursor-pointer border hover:bg-white/10 transition-colors ${analyzingState.isAnalyzing ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-white/5 border-white/10'}`}
          onPointerUp={handleAnalyze}
          whileTap={{ scale: 0.9 }}
        >
